...
class Block(ABC):
    ...
    @classmethod
    @abstractmethod
    def from_stream(cls, reader: TaggedBlockReader):
...
class AuthorIdsBlock(Block):
    ...
    @classmethod
    def from_stream(cls, stream: TaggedBlockReader):
        ...
        return AuthorIdsBlock(author_ids, None)
...
class MigrationInfoBlock(Block):
    ...
    def from_stream(cls, stream: TaggedBlockReader):
        ...
        if stream.bytes_remaining_in_block():
            ...
            unknown = False
        return MigrationInfoBlock(extra_data=b"", migration_id=migration_id,
                                  is_device=is_device, _unknown=unknown)
...
class TreeNodeBlock(Block):
    ...
    def from_stream(cls, stream: TaggedBlockReader):
        ...
        return cls(group=group, extra_data=b'')
    ...
    # def to_stream(self, writer: TaggedBlockWriter):
    #     _logger.debug(f"Writing {type(self).__name__}")
...
class SceneTreeBlock(Block):
    ...
    def from_stream(cls, stream: TaggedBlockReader):
        ...
        return SceneTreeBlock(extra_data="b",
                              tree_id=tree_id,
                              node_id=node_id,
                              is_update=is_update,
                              parent_id=parent_id)
...
class SceneItemBlock(Block):
    ...
    def from_stream(cls, stream: TaggedBlockReader):
        ...
        return subclass(
                parent_id=parent_id,
                item=CrdtSequenceItem(item_id, left_id, right_id, deleted_length, value),
...
def text_item_from_stream(stream: TaggedBlockReader) -> CrdtSequenceItem[Union[str,int]]:
    with stream.read_subblock(0):
        item_id = stream.read_id(2)
        left_id = stream.read_id(3)
        right_id = stream.read_id(4)
...
def text_item_to_stream(item: CrdtSequenceItem[Union[str,int]], writer: TaggedBlockWriter):
    with writer.write_subblock(0):
...
class RootTextBlock(Block):
    ...
    def from_stream(cls, stream: TaggedBlockReader):
        ...
        return RootTextBlock(block_id=block_id, value=value, extra_data=b'')
        ...
        with stream.read_block() as block_info:
            ...
            if block_type:
                try:
                    ...
                    yield UnreadableBlock(error=str(e), data=data, info=block_info, extra_data=b'')
            else:
                msg = (
                ...
                _logger.warning(msg)
                data = stream.data.read_bytes(block_info.size)
                yield UnreadableBlock(error=msg, data=data, info=block_info, extra_data=b'')


def read_blocks(data: Union[str, BinaryIO]) -> Iterator[Block]:
    """
    ...
    for b in blocks:
        if isinstance(b, SceneTreeBlock):
            ...
            if b.group.node_id not in tree:
                raise ValueError(
                    f"Node does not exist for TreeNodeBlock: {b.group.node_id}"
                )
...
#         _logger.debug(f"Writing Point v{version}: {self}")
#         if version == 1:
#             # calculation based on ddvk's reader
...
#         _logger.debug(f"Reading Line version {version}")
#         tool_id = stream.read_int(1)
#         tool = Pen(tool_id)
#         color_id = stream.read_int(2)
...
#     def to_stream(self, writer: TaggedBlockWriter, version: int = 2):
#         _logger.debug(f"Writing Line version {version}")
...
#         "Group item block?"
#         _logger.debug(f"Reading {cls.__name__}")
...
#         return RootTextBlock(extra_data=b"",
#                              block_id=block_id,
...
#             else:
#                 _logger.error(
#                     f"Unknown block type {block_info.block_type}. Skipping {block_info.size} bytes.")
