...
class TaggedBlockReader:
    ...
    def read_subblock(self, index: int) -> Iterator[SubBlockInfo]:
        ...
        subblock = SubBlockInfo(offset=i0, size=subblock_length, extra_data=b"")
        yield subblock
    ...
    def _check_position(self, block_info: BlockInfo):
        ...
        if i1 > i0 + length:
            raise BlockOverflowError(
                f"{type(block_info)} starting at {i0}, length {length}, read up to {i1}(overflow by {i1 - (i0 + length)})"
            )
        if i1 < i0 + length:
